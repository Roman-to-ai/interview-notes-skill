...
> **关于本文档的定位**
>
> 这是一份**示例性的 .docx 工作流参考**，不是使用本项目的必经步骤。
> 若你的题目来源不是 .docx（比如直接手写 Markdown、从网页复制），完全可以跳过本文。
>
> 另外请注意：文中的**方式 1 依赖 Claude Code 的 officecli MCP 技能**，属可选前提；
> 外部环境若未安装该 MCP，请直接使用方式 2 或方式 3（都不需要任何额外依赖）。
> 后文的章节号识别与分组策略假定 .docx 采用「中文数字章节标题（一、二、）+ 数字题号」的排版，
> 你的文档若格式不同，需要相应调整。

## 三种提取方式

### 方式 1：officecli MCP（需 Claude Code + officecli 技能）
...
**前提**：仅在已安装 Claude Code 与 officecli MCP 技能的环境中可用；否则请用方式 2 / 3。
